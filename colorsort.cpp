...
void setMidScoring() {
    ...
    bottomrollers.move_velocity(150);
    midrollers.move_velocity(-100);
...
/**
 * Checks if any roller motor is stuck (commanded velocity high, actual velocity low).
 * If stuck, runs jitter() until the jam is cleared.
 */
void checkAndUnjamMotor() {
    const int STUCK_THRESHOLD = 50; // velocity below this means stuck
    const int COMMAND_THRESHOLD = 400; // commanded velocity above this means should be moving
    const int CHECK_DURATION_MS = 200; // how long to check for stuck

    bool stuck = false;

    // Check bottomrollers
    if (abs(bottomrollers.get_target_velocity()) > COMMAND_THRESHOLD &&
        abs(bottomrollers.get_actual_velocity()) < STUCK_THRESHOLD) {
        stuck = true;
        printf("Bottom roller stuck!\n");
    }
    // Check midrollers
    else if (abs(midrollers.get_target_velocity()) > COMMAND_THRESHOLD &&
             abs(midrollers.get_actual_velocity()) < STUCK_THRESHOLD) {
        stuck = true;
        printf("Mid roller stuck!\n");
    }
    // Check backroller
    else if (abs(backroller.get_target_velocity()) > COMMAND_THRESHOLD &&
             abs(backroller.get_actual_velocity()) < STUCK_THRESHOLD) {
        stuck = true;
        printf("Back roller stuck!\n");
    }

    // If any motor is stuck, run jitter until it's not stuck
    while (stuck) {
        jitter();
        pros::delay(CHECK_DURATION_MS);
...
bool shouldRunUnjamTask = true;

void unjamTaskFunction(void* param) {
    while (shouldRunUnjamTask) {
        checkAndUnjamMotor();
        pros::delay(100); // Check every 100ms
    }
}
void startUnjamTask() {
    pros::Task unjamTask(unjamTaskFunction, nullptr, "Unjam Task");
    // Task runs in background automatically
}
